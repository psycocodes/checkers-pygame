...
class GUI:
    ...
    def draw(self):
        ...
        self.win.blit(title.render(title_txt, True, WHITE), ((self.s_w - title_w)//2,
                                                                 100 + title_h//2))
        title_sub = font_render('f_2', 20)
        title_sub_txt = "AI Implemented"
        title_sub_w, title_sub_h = title_sub.size(title_sub_txt)
        self.win.blit(title_sub.render(title_sub_txt, True, WHITE), ((self.s_w - title_sub_w)//2,
                                                                 150 + title_sub_h//2))
        # Text Rendering
        t_1 = font_render('f_2', 25)
        t_1_txt = 'Difficulty: '
        t_1_w, t_1_h = title_sub.size(t_1_txt)
        self.win.blit(t_1.render(t_1_txt, True, WHITE), (150-t_1_w//2, 235+t_1_h//2))
    ...
    def radio_group_f(self):
        ...
        if selected != self.difficulty and selected is not None:
            if self.sound_manager:
                self.sound_manager.play_click_sound()
        if selected == 1:
            self.difficulty = 1

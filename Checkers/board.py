...
class Board:
    ...
    def draw_squares(win):
        ...
        from GUI.textures import t_tile, tile_x, tile_y
        
        win.blit(t_tile, (tile_x, tile_y))
        
        for row in range(ROWS):
            for col in range(ROWS):
                x = BOARD_X + SQUARE_SIZE * col
                y = BOARD_Y + SQUARE_SIZE * row
                if (row + col) % 2 == 0:
                    win.blit(texture03, (x, y))
                else:
                    win.blit(texture04, (x, y))
    ...
    def draw(self, win):
        ...
        for row in range(ROWS):
            for col in range(COLS):
                ...
                if piece != 0:
                    # Update piece position for centering
                    piece.x = BOARD_X + SQUARE_SIZE * (col + 0.5)
                    piece.y = BOARD_Y + SQUARE_SIZE * (row + 0.5)
                    piece.rect.center = piece.x, piece.y
